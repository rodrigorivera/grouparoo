...
export const importRecordsInGroups: RecordGroupRecordsPluginMethod = async (
  imports,
  config,
  methods
) => {
  ...
  const users: GroupImport[] = createGroupImports(imports, methods, config);
...
function convertImportedRecord(importedRecord: ImportedRecord): GroupImport {
  const user: GroupImport = Object.assign({}, importedRecord);
  return user;
}

function addToMap(
  user: GroupImport,
  userMap: { [fkValue: string]: GroupImport }
) {
  if (!user) {
    return;
  }
...
function createGroupImports(
  importedRecords: ImportedRecord[],
  methods: GroupMethods,
  config: GroupConfig
): GroupImport[] {
  const userMap: { [fkValue: string]: GroupImport } = {};
  const errors: GroupImport[] = [];
  for (const importedRecord of importedRecords) {
    try {
      ...
      const user = convertImportedRecord(importedRecord);
      user.error = error;
      errors.push(user);
    }
  }
...
function separateForeignKeys(
  ...
  methods: GroupMethods,
  config: GroupConfig
): { newUser: GroupImport; oldUser: GroupImport } {
  ...
  const newUser = convertImportedRecord(importedRecord);
  ...
  if (oldValue) {
    ...
    if (newValue !== oldValue && oldValue.length > 0) {
      oldUser = convertImportedRecord(importedRecord);
...
async function updateGroups(
  ...
  for (const user of users) {
    ...
    if (syncOperations.delete) {
      ...
      const message = "Source is not removing.";
    ...
    if (syncOperations.create) {
      ...
      const message = "Source is not adding.";
      skipMessage = skipMessage ? `${skipMessage} ${message}` : message;
    }
