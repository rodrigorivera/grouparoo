...
export const getSourceOptions: SalesforceSourceOptions = async ({
  ...
  if (which.record) {
    Object.assign(
      out,
      await getRecordOptions(conn, cacheData, sourceOptions)
    );
  }
  if (which.group) {
    Object.assign(
      out,
      await getGroupOptions(conn, cacheData, sourceOptions)
    );
  }
  if (which.membership) {
    Object.assign(
      out,
      await getMembershipOptions(conn, cacheData, sourceOptions)
    );
  }
  if (which.reference) {
    Object.assign(
      out,
      await getReferenceOptions(conn, cacheData, sourceOptions)
    );
  }
  return out;
};
...
async function getRecordOptions(
  ...
  const name = sourceOptions.recordObject?.toString();
  if (name && objects.includes(name)) {
    ...
    if (!fields.includes(sourceOptions.recordMatchField)) {
      sourceOptions.recordMatchField = null;
    }
  } else {
    sourceOptions.recordObject = null;
    sourceOptions.recordMatchField = null;
  }
...
async function getGroupOptions(
  ...
  const name = sourceOptions.groupObject?.toString();
  if (name && objects.includes(name)) {
    // look up its fields
    ...
    if (!fields.includes(sourceOptions.groupNameField)) {
      sourceOptions.groupNameField = null;
    }
  } else {
    sourceOptions.groupObject = null;
    sourceOptions.groupNameField = null;
  }
...
async function getMembershipOptions(
  ...
  const name = sourceOptions.membershipObject?.toString();
  if (name && objects.includes(name)) {
    // look up its fields
    const fields = await getObjectMatchNames(
      conn,
    ...
    if (!fields.includes(sourceOptions.membershipRecordField)) {
      sourceOptions.membershipRecordField = null;
    }

    out.membershipGroupField.type = "typeahead";
    out.membershipGroupField.options = fields;
    if (!fields.includes(sourceOptions.membershipGroupField)) {
      sourceOptions.membershipGroupField = null;
    }
  } else {
    sourceOptions.membershipObject = null;
    sourceOptions.membershipRecordField = null;
    sourceOptions.membershipGroupField = null;
  }
  return out;
...
async function getReferenceOptions(
  ...
  const recordObject = sourceOptions.recordObject?.toString();
  ...
  const fieldName = sourceOptions.recordReferenceField?.toString();
  if (fieldName && nameMap[fieldName]) {
    ...
    const refName = sourceOptions.recordReferenceObject?.toString();
    if (refName && relationshipObjects.includes(refName)) {
      ...
      if (!refFields.includes(sourceOptions.recordReferenceMatchField)) {
        sourceOptions.recordReferenceMatchField = null;
      }
    } else {
      sourceOptions.recordReferenceObject = null;
      sourceOptions.recordReferenceMatchField = null;
    }
  } else {
    sourceOptions.recordReferenceField = null;
    sourceOptions.recordReferenceObject = null;
    sourceOptions.recordReferenceMatchField = null;
  }
