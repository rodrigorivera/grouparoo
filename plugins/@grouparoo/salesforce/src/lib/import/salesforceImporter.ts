...
import {
  ...
  BatchMethodCreateByForeignKeyAndSetSourceIds,
  BatchMethodDeleteBySourceIds,
  BatchMethodFindAndSetSourceIds,
  BatchMethodGetClient,
  BatchMethodNormalizeForeignKeyValue,
  BatchMethodNormalizeGroupName,
  BatchMethodRemoveFromGroups,
  BatchMethodUpdateBySourceIds,
  importRecordsInBatch,
...
const findAndSetSourceIds: BatchMethodFindAndSetSourceIds = async ({
  client,
  users,
  foreignKeys,
  getByForeignKey,
...
const deleteBySourceIds: BatchMethodDeleteBySourceIds = async ({
  client,
  users,
  config,
}) => {
  const { recordObject } = config.data;
  const payload = users.map((user) => user.sourceId);
...
async function buildPayload(
  ...
  users: BatchImport[],
  config: BatchConfig
): Promise<{ usersInPayload: BatchImport[]; payload: any[] }> {
  ...
  const payload = [];
  const usersInPayload: BatchImport[] = [];
...
function processResults(results, users: BatchImport[], type: ResultType) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    try {
      ...
      if (type === ResultType.USER) {
        if (user.sourceId && user.sourceId !== id) {
          throw new Error(
            `sourceId does not match updated one: ${user.sourceId} -> ${id}`
          );
        }
        user.sourceId = id;
      }
        ...
        if (type === ResultType.USER) {
          if (!user.sourceId) {
            ...
            user.error = new Error(
              `sourceId never set from ok process error: ${user.sourceId}`
            );
          }
...
const updateBySourceIds: BatchMethodUpdateBySourceIds = async ({
  client,
  users,
  config,
}) => {
...
const createByForeignKeyAndSetSourceIds: BatchMethodCreateByForeignKeyAndSetSourceIds =
...
const addToGroups: BatchMethodAddToGroups = async ({
  ...
  for (const name in groupMap) {
    ...
    for (const importedRecord of records) {
      payload.push({
        [membershipGroupField]: id,
        [membershipRecordField]: importedRecord.sourceId,
      });
      users.push(importedRecord);
    }
  }
...
const removeFromGroups: BatchMethodRemoveFromGroups = async ({
  ...
  for (const name in groupMap) {
    const id = await getListId(client, name, config.data);
    const records = groupMap[name] || [];
    const destIds = records.map((pro) => pro.sourceId);
    if (destIds.length > 0) {
      const query = {
        [membershipGroupField]: id,
      ...
      for (const result of results) {
        ...
        if (!destId) {
          throw new Error(`no source id: ${JSON.stringify(result)}`);
        }
...
export interface ImportSalesforceMethod {
  (argument: {
    appId: string;
...
export const importSalesforceBatch: ImportSalesforceMethod = async ({
  appId,
  appOptions,
  sourceOptions,
  syncOperations,
  ...
  imports.map((currentImport) => {
    if (
      !sourceOptions.groupObject ||
      !sourceOptions.groupNameField ||
      !sourceOptions.membershipObject ||
      !sourceOptions.membershipRecordField ||
      !sourceOptions.membershipGroupField
    ) {
      currentImport.oldGroups = [];
      currentImport.newGroups = [];
    }
  });
  ...
  return importRecordsInBatch(
    imports,
    {
      ...
      sourceOptions: null, // in data now
    },
    {
      getClient,
      findAndSetSourceIds,
      deleteBySourceIds,
      updateBySourceIds,
      createByForeignKeyAndSetSourceIds,
      addToGroups,
      removeFromGroups,
