import {
  SourceMappingOptionsResponseType,
  SimpleAppOptions,
  SourceMappingOptionsMethodResponse,
  Source,
} from "@grouparoo/core";
...
import { SourceSyncModeData } from "@grouparoo/core/dist/models/Source";

export interface SalesforceSourceMappingOptionsMethod {
  (argument: {
    source: Source;
    appOptions: SimpleAppOptions;
    appId: string;
    model: SalesforceModel;
  }): Promise<SourceMappingOptionsMethodResponse>;
}

export const getSourceMappingOptions: SalesforceSourceMappingOptionsMethod =
  async ({ appId, appOptions, source, model }) => {
...
const extractFields = async (
  ...
  required: {
    key: string;
    type: SourceMappingOptionsResponseType;
  }[];
  known: {
    key: string;
    type: SourceMappingOptionsResponseType;
  ...
  for (const field of info.fields) {
    ...
    if (!field.nillable && !field.defaultedOnCreate) {
      ...
      const syncMode = await source.getSyncMode();
      if (syncMode === SourceSyncModeData.enrich.key) {
